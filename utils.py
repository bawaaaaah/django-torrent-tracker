...
def genslug():
    import random
    return ''.join(random.sample('%s%s'%(string.digits, string.lowercase), 20))

def validate_image(data):
    error_messages = {
	'invalid_image': _(u"Upload a valid image. The file you uploaded was either not an image or a corrupted image."),
	'improper_format': _(u"Upload GIF, JPEG or PNG.")
    }
    ...
    try:
        ...
        raise ValidationError(error_messages['invalid_image'])
    if trial_image.format != 'JPEG' and trial_image.format != 'PNG' and trial_image.format != 'GIF':
	raise ValidationError(error_messages['improper_format'])
    return data 

def mkstemp(suffix, preffix, d, data):
    from settings import MEDIA_ROOT
    from tempfile import mkstemp
    d = os.path.join(MEDIA_ROOT, d)
    f, n = mkstemp(suffix, preffix, d)
    os.write(f, data)
    os.close(f)
    return os.path.basename(n)

def save_imgs(data, uid, field):
    from settings import MEDIA_ROOT
    from PIL import Image
    from cStringIO import StringIO
    import random
	...
	img = img.convert('RGB')
    while True:
	fn = '%s_%s.%s'%(uid, ''.join(random.sample(letters+digits, 8)), format)
	if not os.path.exists(os.path.join(MEDIA_ROOT, field, fn)):
	    break
    fd = open('%s/%s'%(os.path.join(MEDIA_ROOT, '%s'%field), fn), 'wb')
    img.save(fd)
    img = Image.open(StringIO(data))
    img.thumbnail((122,122), Image.ANTIALIAS)
    img.save('%s/%s'%(os.path.join(MEDIA_ROOT, 'i%s'%field), fn), img.format)
    return fn
...
def sanitize_tags(tags):
	...
	if len(tag_name) > settings.MAX_TAG_LENGTH:
	    #raise ValidationError('Each tag may be no more than %s characters long.' % settings.MAX_TAG_LENGTH)
	    return ''
	tn = tag_name.replace('&#38;', 'and')
	tn = re.sub('&.*?;', '', tn, re.UNICODE)
	tn = re.sub('/', ',',tn, re.UNICODE)
	tn = tn.strip()
	tn = re.sub('['+ punctuation + ']+', ' ', tn, re.UNICODE)
	outp.append(tn)
    return ','.join(outp)

def iri_to_uri_2396(iri):
    import urllib
...
class HttpResponseRedirect(HttpResponse):
	...
	self['Location'] = iri_to_uri_2396(redirect_to)

def order_by(sortlist, orderby=[], desc=[]):
    from operator import itemgetter
    for i in reversed(orderby):
	sortlist.sort(key=itemgetter(i), reverse=(i in desc))
    return sortlist
...
def reverse(viewname, *args, **kwargs):
    ...
    args = args or []
    kwargs = kwargs or {}
    return iri_to_uri_2396(u'/' + get_resolver(kwargs.get('urlconf', None)).reverse(viewname, *args, **kwargs))
