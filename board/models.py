...
from board import managers
from board.middleware import threadlocals

from fields import Dict

__all__ = [
    'NOBODY', 'ALL', 'USERS', 'CUSTOM', 'PERM_CHOICES', 'PERM_CHOICES_RESTRICTED',
    'PermissionError', 'Category', 'Invitation', 'Group', 'Thread', 
    'Post', 'Moderator', 'WatchList', 'AbuseReport', 'UserSettings', 
    'IPBan', 'UserBan',
    ]

_log = logging.getLogger('board.models')
...
class Invitation(models.Model):
    ...
    group = models.ForeignKey(Group, verbose_name=_('group'),
        related_name='sb_invitation_set')
    sent_by = models.ForeignKey(User, verbose_name=_('sent by'),
        related_name='sb_sent_invitation_set')
    sent_to = models.ForeignKey(User, verbose_name=_('sent to'),
        related_name='sb_received_invitation_set')
    sent_date = models.DateTimeField(_('sent date'), auto_now_add=True)
    response_date = models.DateTimeField(_('response date'), blank=True,
        null=True)
...
class Thread(models.Model):
    ...
    def get_url(self):
        return reverse('board_thread', args=(self.id,))
...
class Post(models.Model):
    ...
    private = models.ManyToManyField(User, related_name="sb_private_posts_set",
        null=True, verbose_name=_('private recipients'))
    ...
    def get_absolute_url(self):
        return reverse('board_locate_post', args=(self.id,))
...
class AbuseReport(models.Model):
    ...
    seen = models.BooleanField(default=False, help_text="<a href=\"post\">View post on forum.</a>")
...
#            help_text = "Email notifications for watched discussions.", verbose_name=_('notify'))
    reverse_posts = models.BooleanField(
            default=False, help_text = _('Display newest posts first.'), 
            verbose_name=_('new posts first'))
...
class UserBan(models.Model):
    ...
    user = models.ForeignKey(User, unique=True, verbose_name=_('user'),
        db_index=True, related_name='sb_userban_set',
