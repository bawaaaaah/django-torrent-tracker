...
_log = logging.getLogger('board.managers')
...
class ThreadManager(models.Manager):
    def get_query_set(self):
        ...
        extra_post_count = """
            SELECT COUNT(1) FROM board_post
                ...
                AND board_post.revision_id IS NULL
                AND NOT board_post.censor
            """
        ...
        try:
            ...
            if us.frontpage_filters.count():
                return self.get_query_set().filter(
                    category__id__in=[k['id'] \
                        for k in us.frontpage_filters.all().values('id')]
                )
    ...
    def get_first_post_text(self, thread_id, user):
        from board.models import Post
        result = Post.view_manager.posts_for_thread(thread_id, user)[:1]
        if result:
...
class CategoryManager(models.Manager):
    def get_query_set(self):
        thread_count = """
            SELECT COUNT(1) FROM board_thread, board_category
            WHERE board_thread.category_id = board_category.id
            """
