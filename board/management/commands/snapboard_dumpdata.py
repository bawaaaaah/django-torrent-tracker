...
from board import models, __version__

def _get_board_objects():
...
class Command(BaseCommand):
    '''
    Dump all the board data in the database to stdout. All of Django's 
    ...
    the current board version identifier.

    This is useful between releases where board's model definitions 
    ...
    help = 'Dump all the board data in the database to stdout. All of '\
            'Django\'s authentication framework\'s User instances are also '\
            'included.'

    def handle(self, *args, **kwargs):
        s = get_serializer('xml')()
        s.serialize(_get_board_objects(), **kwargs)
        s.stream.seek(0)
        sys.stdout.write(s.stream.read(len('<?xml version="1.0" encoding="utf-8"?>\n')))
        sys.stdout.write('''<boardDataDump version="%s">''' % __version__)
        for chunk in iter(s.stream):
            sys.stdout.write(chunk)
        sys.stdout.write('</boardDataDump>')
