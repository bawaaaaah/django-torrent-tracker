...
class boardDeserializer(Deserializer):
    ...
    def next(self):
        for event, node in self.event_stream:
            if event == "START_ELEMENT":
                ...
                elif node.nodeName == "boardDataDump":
...
class Command(BaseCommand):
    '''
    Load data dumped by board_dumpdata. Data dumped by earlier 
    releases of board is automatically converted if required.
    '''
    ...
    help = 'Load data dumped by board_dumpdata. Data dumped by earlier '\
           'releases of board is automatically converted if required.'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            try:
                objects = list(boardDeserializer(f))
